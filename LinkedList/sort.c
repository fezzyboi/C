...
int SortedWordCount(ListItem *list, int *wordCount)
{
    ...
    while (list != NULL) {
        if (list->data == NULL) {
            ...
            int counter = 1;
            ListItem *item1 = list;
            while (item1->nextItem != NULL && strcmp(item1->data, item1->nextItem->data) == 0) {
                counter++;
                item1 = item1->nextItem;
            }
            list=item1;
...
int main()
{
    ...
    ListItem *sortedWordList = LinkedListGetFirst(unsortedWordList);
    LinkedListSort(sortedWordList);
    LinkedListPrint(sortedWordList);
    if (SortedWordCount(sortedWordList, wordCount)) {
        printf("[%d, %d, %d, %d, %d, %d, %d, %d, %d, %d]\n",
                wordCount[0], wordCount[1], wordCount[2],
                wordCount[3], wordCount[4], wordCount[5],
                wordCount[6], wordCount[7], wordCount[8],
    ...
    free(unsortedWordList);
    while (sortedWordList != NULL) {
        LinkedListRemove(sortedWordList);
        sortedWordList = sortedWordList->nextItem;
    }
    free(sortedWordList);
 ...
 * @return Either 1 or 0 if the head of the doubly-linked list isn't passed.
 */
 ...
 * @return 1 if it succeeds, 0 if it fails to allocate necessary memory.
 */
